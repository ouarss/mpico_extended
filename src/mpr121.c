...
{
    return mpr121_read_many(addr, MPR121_BASELINE_VALUE_REG, baseline, num);
}
...
// Global analog front-end gain: CDC = charge/discharge current (0x5C[5:0]),
// CDT = charge/discharge time (0x5D[7:5]). Preserve the filter bits that share
// these registers (FFI in 0x5C[7:6], SFI/ESI in 0x5D[4:0]).
void mpr121_gain(uint8_t addr, uint8_t cdc, uint8_t cdt)
{
    uint8_t ecr = mpr121_stop(addr);

    uint8_t afe = read_reg(addr, MPR121_AFE_CONFIG_REG);
    write_reg(addr, MPR121_AFE_CONFIG_REG, (afe & 0xC0) | (cdc & 0x3f));
    uint8_t fcr = read_reg(addr, MPR121_FILTER_CONFIG_REG);
    write_reg(addr, MPR121_FILTER_CONFIG_REG, (fcr & 0x1f) | ((cdt & 0x07) << 5));
