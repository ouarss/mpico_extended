...
static const uint8_t button_led_def[] = RGB_BUTTON_MAP;

#define BUTTON_LED_NUM (sizeof(button_led_def))

/* Effective LED position for a button: runtime override, or hardcoded default */
static uint8_t button_led_pos(unsigned index)
{
    uint8_t pos = mai_cfg->alt.rgb_button[index];
    if (pos >= BUTTON_LED_NUM) {
        pos = button_led_def[index];
    }
    return pos;
}
...
{
    if (index >= BUTTON_LED_NUM) {
        return 0xff;
    }
    return button_led_pos(index);
}
...
void rgb_set_button(unsigned index, uint32_t color, uint8_t speed)
{
    ...
    set_color(button_led_pos(index), color, speed);
}
